...
func (input *UCGetBackupTransfersListInput) Valid() error {
	const (
		int0Key      = 0
		int1Key      = 1
		intMinus1Key = -1
	...
	return validation.ValidateStruct(input,
		...
		validation.Field(&input.Pagination, validation.By(func(value interface{}) error {
			...
			pagination, ok := value.(UCGetBackupTransfersListPaginationInput)
			if !ok {
			...
			return validation.ValidateStruct(&pagination,
				...
				validation.Field(&pagination.Cursor, validation.By(func(value interface{}) error {
					...
					var cursor UCGetBackupTransfersListCursorBase
					cursor, ok = value.(UCGetBackupTransfersListCursorBase)
					if !ok {
						return ErrValueInvalid
					}
